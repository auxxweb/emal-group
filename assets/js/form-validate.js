	...
	//}else if( f.phone.value == '' ){
		...
		notice( f.message );
		 ...
		 jQuery.ajax({
			url: 'mail.php',
			type: 'post',
			data: jQuery('form#Emal Group-contact').serialize(),
			complete: function(data) {
				jQuery('#form_status').html(data.responseText);
				jQuery('#Emal Group-contact').find('input,textarea').attr({value:''});
				jQuery('#Emal Group-contact').css({opacity:1});
				jQuery('#Emal Group-contact').remove();
			}
		});
		jQuery('#form_status').html('<span class="loading">Sending your message...</span>');
		jQuery('#Emal Group-contact').animate({opacity:0.3});
		jQuery('#Emal Group-contact').find('input,textarea,button').css('border','none').attr({'disabled':''});
	}
...
function notice( f ){
	jQuery('#Emal Group-contact').find('input,textarea').css('border','none');
